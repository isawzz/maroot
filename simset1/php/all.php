...
if ($data['cmd'] === 'savey' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if ($data['cmd'] === 'dir' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $dir= $data['dir'] ?? '';
    ...
    echo json_encode(["data" => $data, "dir" => $dir, "filesInfo" => $filesInfo, "files" => $files]);
    exit;
}

if ($data['cmd'] === 'deletey' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $path = YDIR . $data['file'] . '.yaml';
    unlink($path);
...
if ($data['cmd'] === 'delete_dir' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if ($data['cmd'] === 'test_array' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if ($data['cmd'] === 'test_config' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if ($data['cmd'] === 'test_final' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    //echo json_encode(["users" => USERS_READ]); die;
    $username = 'dieter';
    $userdata = new UserData($username);
    $users = yamlFileToArray(USERS_READ);
...
if ($data['cmd'] === 'test' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $jsonString = '{"name":"Alice","age":30,"address":{"street":"123 Maple St","city":"New York"}}';
    $php = json_decode($jsonString, true);
    $s = json_encode($php, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
    $yamlString = jsonToYaml($s);
