...
class AbstractTile extends ValueBlend {
	...
	// }
	_u_flip(index_keys) {
		// Removed the !this.u_symmetric check to allow 
		// boundary handshaking for asymmetric tiles.
		return this._swap_value(index_keys, 'left', 'right');
	}

	_v_flip(index_keys) {
		// Removed the !this.v_symmetric check to allow 
		// boundary handshaking for asymmetric tiles.
		return this._swap_value(index_keys, 'bottom', 'top');
	}
...
class Tile extends AbstractTile {
	...
	set_equivalent_vert(neighbor_keys, index_keys, vert, options = {}) {
		...
		//console.log(tile,neighbor_keys,index_keys,vert)
		if (tile === null) {
			return null;
		}
	...
	num_color_patterns() {
		return this.tessagon.constructor.num_color_patterns();
	}
	calculate_colors() {
	...
	_set_u_equivalent_vert(index_keys, vert, options = {}) {
		...
		//console.log(index_keys,u_index,u_flip_keys)
	...
	_set_v_equivalent_vert(index_keys, vert, options = {}) {
		...
		//console.log('____',index_keys,v_index,v_flip_keys)
		let res = this.set_equivalent_vert([v_index], v_flip_keys, vert, options);
...
class GridTileGenerator extends TileGenerator {
	...
	initialize_tiles(options = {}) {
		let tiles = [];
		let opts = addKeys(this.tessagon.options, options);
		//console.log('______________', opts)
		...
		for (let u = 0; u < this.u_num; u++) {
			...
			for (let v = 0; v < this.v_num; v++) {
				// // --- ADD THIS FILTER CHECK ---
				// console.log('initialize',u,v)
				// if (options.tile_filter && !options.tile_filter(u, v)) {
				// 	continue;
				// }
				// // -----------------------------			
	...
	initialize_neighbors(options = {}) {
		...
		for (let u = 0; u < this.u_num; u++) {
			...
			for (let v = 0; v < this.v_num; v++) {
				...
				let tile = tiles[u][v];
				if (!tile) continue; // Skip tiles that were filtered out
	...
	get_tiles() {
		...
		//return this.tiles.flat().filter(tile => tile !== null);
		const flattened = [];
		for (let v = 0; v < this.v_num; v++) {
			for (let u = 0; u < this.u_num; u++) {
				const tile = this.tiles[v][u];
				if (tile) {
					flattened.push(tile);
				}
			}
...
class SvgAdaptor extends ListAdaptor {
	static ADAPTOR_OPTIONS = [
		'rotate90', 'svg_id', 'svg_root_tag', 'svg_style', 'svg_fill_color',
		'svg_fill_colors', 'svg_stroke_color', 'svg_stroke_width',
		'color_method' // New option: 'palette', 'hsl', or 'custom'
	];

	constructor(options = {}) {
		super(options);
		this.rotate90 = options.rotate90 || false;
		this.svg_id = options.svg_id || 'tessellation_svg';
		this.svg_root_tag = options.svg_root_tag || false;
		...
		// Default to 'palette' if colors are provided, otherwise 'hsl'
		this.color_method = options.color_method || (this.svg_fill_colors ? 'palette' : 'hsl');
	}
	_create_vert(coords) {
		//console.log(coords)
		this.vert_list.push(coords);
	...
	create_vert(coords) {
		...
		if (this.rotate90) {
			...
			// The new Y is the inverse of the old X
			final_u = v;
			final_v = 1.0 - u;
		}

		// Return the coordinates for the SVG points attribute
		return this._create_vert([final_u, final_v]);
	}

	get_mesh() {
		let o = super.get_mesh();
		o.color_method = this.color_method;
		let buffer = "";
		if (this.svg_root_tag) {
			if (this.svg_root_tag === true) {
				// Use the dynamic ID here
				buffer += `<svg id="${this.svg_id}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1">\n`;
			} else {
				buffer += this.svg_root_tag;
			}
		}
		buffer += `<g${this.group_style()}>\n`;
		if (this.style) buffer += `<style>${this.style}</style>`;

		// Loop through all faces
		for (let i = 0; i < this.face_list.length; i++) {
			const face = this.face_list[i];
			const colorIndex = this.color_list[i] || 0;

			// We pass everything make_face needs to decide its own color
			buffer += this.make_face(face, i, colorIndex);
		}

		buffer += "</g>\n";
		if (this.svg_root_tag) buffer += "</svg>\n";
		return { mesh: o, svg: buffer };
	}
	group_style() {
		let style = "";
	...
	make_face(face, id, colorIndex) {
		const verts = face.map(v => this.vert_list[v]);
		const points_string = verts.map(vert => `${vert[0]},${vert[1]}`).join(" ");

		// Calculate center for spatial coloring (HSL)
		const centerX = verts.reduce((sum, v) => sum + v[0], 0) / verts.length;
		const centerY = verts.reduce((sum, v) => sum + v[1], 0) / verts.length;

		let fillStr = "";

		// Strategy Selection
		if (this.color_method === 'hsl') {
			const hue = centerX * 360;
			const lightness = 40 + (centerY * 40);
			fillStr = `fill="hsl(${hue}, 70%, ${lightness}%)" `;
		} else if (this.color_method === 'palette' && this.svg_fill_colors) {
			// Use the palette, fallback to first color if index is out of bounds
			const color = this.svg_fill_colors[colorIndex % this.svg_fill_colors.length];
			fillStr = `fill="${color}" `;
		} else if (this.color_method === 'solid' || this.svg_fill_color) {
			// Fallback to the single solid color provided
			fillStr = `fill="${this.svg_fill_color || '#ccc'}"`;
		}
      ...
      data-cx="${centerX.toFixed(3)}"
      data-cy="${centerY.toFixed(3)}"
    />\n`;
	}
}
...
class FloretTile extends AlternatingTile {
	...
	color_pattern3() {
		...
		for (const face in this.faces) {
			const fp = this.floret_fingerprint(face);
			// Fix: ensure the row index is non-negative for negative fingerprints
			const row = (fp[1] % 2 + 2) % 2;
			const column = ((fp[0] - 2 * fp[1]) % 6 + 6) % 6;
			this.color_face(face, pattern[row][column]);
		}
	}
}
...
class HexSquareTriTile extends Tile {
	...
	calculate_verts() {
		const u_unit = 1.0 / (1.0 + Math.sqrt(36)); //3
		const u0 = 0;
		const u1 = 0.5 * u_unit;
		const u2 = 0.5 * (1.0 - u_unit);
		const u3 = 0.5;
		const v_unit = 1.0 / (3.0 + Math.sqrt(.2)); //3
...
class WeaveTile extends Tile {
	constructor(tessagon, options = {}) {
		...
		this.u_symmetric = true;
		this.v_symmetric = true;
		// ratio: 0.5 is thin, 0.7-0.8 looks like a tight weave
		this.square_ratio = options.hasOwnProperty("square_ratio") ? options.square_ratio : 0.6;
	}

	init_verts() {
		// We must define every level of the path used in add_vert
		// Paths are 4 levels: [top/bottom, left/right, u_inner/outer, v_inner/outer]
		const inner_structure = () => ({
			u_inner: { v_inner: null, v_outer: null },
			u_outer: { v_inner: null, v_outer: null }
		});

		return {
			top: {
				left: inner_structure(),
				right: inner_structure()
			},
			bottom: {
				left: inner_structure(),
				right: inner_structure()
	...
	calculate_verts() {
		...
		// Symmetry handles the other 3 quadrants automatically
	...
	calculate_faces() {
		// 1. The small squares in the corners of the "weave"
		this.add_face(["square", "top", "left"], [
		...
		// 2. The central horizontal/vertical interlaced strip
		this.add_face(["oct", "middle", "center"], [
			["top", "left", "u_outer", "v_inner"],
			["top", "left", "u_inner", "v_inner"],
		...
		// 3. Boundary strips that connect tiles
		this.add_face(["oct", "top", "left"], [
			["top", "left", "u_inner", "v_outer"],
			["top", "left", "u_outer", "v_outer"],
	...
	calculate_colors() {
		if (this.color_pattern === 1) {
			this.color_pattern_alternating(); // Our new checkerboard
		} else if (this.color_pattern === 2) {
			this.color_pattern_directional(); // Our new directional weave
		} else if (this.color_pattern === 3) {
			this.color_pattern_weave_focus(); // Our new directional weave
		} else {
			this.color_pattern_standard(); // Your original logic (Pattern 0)
		}
	}
	color_pattern_standard() {
	...
	// New Pattern: Checkerboard parity
	color_pattern_alternating() {
		let parity = (this.u_index + this.v_index) % 2;
		this.color_face(["oct", "top", "center"], parity);
		this.color_face(["oct", "middle", "left"], parity);
		this.color_face(["oct", "top", "left"], 1 - parity);
		this.color_face(["oct", "middle", "center"], 1 - parity);
		this.color_face(["square", "top", "left"], 2); // Distinct corner squares
	}

	// New Pattern: Warp vs Weft (Vertical vs Horizontal)
	color_pattern_directional() {
		this.color_face(["oct", "top", "center"], 1);    // Vertical strips
		this.color_face(["oct", "middle", "left"], 2);   // Horizontal strips
		this.color_face(["oct", "top", "left"], 0);      // Corners
		this.color_face(["oct", "middle", "center"], 0); // Intersection
	}
	color_pattern_weave_focus() {
		// 1. The central 'main' intersection
		this.color_face(["oct", "middle", "center"], 0);

		// 2. The vertical strand segments 
		// (In your tile, these are the top/bottom center octs)
		this.color_face(["oct", "top", "center"], 1);

		// 3. The horizontal strand segments 
		// (In your tile, these are the middle left/right octs)
		this.color_face(["oct", "middle", "left"], 2);

		// 4. The corner 'octs' (the ones that touch multiple tiles)
		this.color_face(["oct", "top", "left"], 3);

		// 5. The small corner squares
		this.color_face(["square", "top", "left"], 4);
	}
	color_pattern1() {
		this.color_pattern_weave_focus();
	}
}
...
//#region new code
class SimpleBoundaryTile extends Tile {
  constructor(tessagon, options = {}) {
    super(tessagon, options);
    this.u_symmetric = true;
    this.v_symmetric = true;
  ...
  calculate_verts() {
    ...
    v = this.add_vert(['left'], 0.0, 0.5, { u_boundary: true });
    v = this.add_vert(['bottom'], 0.5, 0.0, { v_boundary: true });
  }

  calculate_faces() { }
}
class SimpleBoundaryTessagon extends Tessagon {
  static tile_class = SimpleBoundaryTile;
  static metadata = new TessagonMetadata({
    name: "Simple Boundary Points",
...
class Simple2Tile extends SimpleBoundaryTile {
  constructor(tessagon, options = {}) {
    super(tessagon, options);
    this.u_symmetric = false;
    this.v_symmetric = false;
  ...
  calculate_verts() {
    let v;
    let [j0, j1, j2, j3] = [1 / 3, .5, 1 / 3, .5]; //[rNumber(.3,.7),rNumber(.4,.6),rNumber(.4,.6),rNumber(.4,.6)]; //.5;//rNumber(.4,.6);
    v = this.add_vert(['left', 'B'], 0.0, j0, { u_boundary: true });
    v = this.add_vert(['left', 'A'], 0.0, j0 * 2, { u_boundary: true });
    v = this.add_vert(['bottom', 'A'], j1, 0.0, { v_boundary: true });
    v = this.add_vert(['right', 'B'], 1.0, j2, { u_boundary: true });
    v = this.add_vert(['right', 'A'], 1.0, j2 * 2, { u_boundary: true });
    v = this.add_vert(['top', 'A'], j3, 1.0, { v_boundary: true });
  }
  ...
  color_pattern1() {
    console.log('applying color pattern 1')
    // Add U and V indices to check if the tile is "even" or "odd"
    if ((this.fingerprint[0] + this.fingerprint[1]) % 2 === 0) {
      // Even tiles: Middle hexagon is Color 1, Inter quad is Color 0
      this.color_paths([["middle"]], 1, 0);
    } else {
      // Odd tiles: Inter quad is Color 1, Middle hexagon is Color 0
      this.color_paths([["inter"]], 1, 0);
    }
  }
  color_pattern2() {
    const u = this.fingerprint[0];

    if (u % 3 === 0) {
      this.color_paths_hash({
        1: [["middle"]], // Middle is Color 1
        2: [["inter"]]   // Inter is Color 2
      }, 0);
    } else if (u % 3 === 1) {
      this.color_paths_hash({
        2: [["middle"]], // Middle is Color 2
        0: [["inter"]]   // Inter is Color 0
      }, 1);
    } else {
      this.color_paths_hash({
        0: [["middle"]], // Middle is Color 0
        1: [["inter"]]   // Inter is Color 1
      }, 2);
    }
  }
  color_pattern3() {
    let u = this.fingerprint[0];
    let v = this.fingerprint[1];
    u=u+v;let mod=4;

    if (u % mod === 0) {
      this.color_paths_hash({
        0: [["middle"]], // Middle is Color 1
        2: [["inter"]]   // Inter is Color 2
      }, 0);
    } else if (u % mod === 1) {
      this.color_paths_hash({
        1: [["middle"]], // Middle is Color 2
        3: [["inter"]]   // Inter is Color 0
      }, 1);
    } else if (u % mod === 2) {
      this.color_paths_hash({
        2: [["middle"]], // Middle is Color 2
      ...
      this.color_paths_hash({
        3: [["middle"]], // Middle is Color 0
        1: [["inter"]]   // Inter is Color 1
      }, 3);
    }
  }
}
class Simple2Tessagon extends SimpleBoundaryTessagon {
  static tile_class = Simple2Tile;
  static metadata = new TessagonMetadata({
    name: "Simple Boundary Points",
    num_color_patterns: 3,
    classification: "debug",
...
class Simple1Tile extends SimpleBoundaryTile {
  constructor(tessagon, options = {}) {
    super(tessagon, options);
    this.u_symmetric = false;
    this.v_symmetric = false;
  ...
  calculate_verts() {
    let v;
    let [j0, j1, j2, j3] = [.5, .5, .5, .5]; //[rNumber(.3,.7),rNumber(.4,.6),rNumber(.4,.6),rNumber(.4,.6)]; //.5;//rNumber(.4,.6);
    v = this.add_vert(['left'], 0.0, j0, { u_boundary: true });
    v = this.add_vert(['bottom'], j1, 0.0, { v_boundary: true });
    v = this.add_vert(['right'], 1.0, j2, { u_boundary: true });
    v = this.add_vert(['top'], j3, 1.0, { v_boundary: true });
  }

  calculate_faces() { }
}
class Simple1Tessagon extends SimpleBoundaryTessagon {
  static tile_class = Simple1Tile;
  static metadata = new TessagonMetadata({
    name: "Simple Boundary Points",
    num_color_patterns: 0,
    classification: "debug",
...
function attachHoverHighlight(svgElement, neighborMap) {
  const faces = svgElement.querySelectorAll('.tess-face');
  ...
  faces.forEach(face => {
    face.addEventListener('mouseenter', () => {
      ...
      neighbors.forEach(nIdx => {
        const nFace = svgElement.querySelector(`[data-index="${nIdx}"]`);
        if (nFace) nFace.classList.add('is-neighbor');
      });
    });
    ...
    face.addEventListener('mouseleave', () => {
      ...
      faces.forEach(f => {
        f.classList.remove('is-active', 'is-neighbor');
      });
    });
  });
}

function getFaceNeighbors(faceList) {
	const vertexToFaces = {};
	const neighborMap = {};

	// Map each vertex to the faces that use it
	faceList.forEach((face, faceIdx) => {
		face.forEach(vertIdx => {
			if (!vertexToFaces[vertIdx]) vertexToFaces[vertIdx] = [];
			vertexToFaces[vertIdx].push(faceIdx);
		});
	});

	// Two faces are neighbors if they share 2 or more vertices (an edge)
	faceList.forEach((face, faceIdx) => {
		const potentialNeighbors = {};
		face.forEach(vertIdx => {
			vertexToFaces[vertIdx].forEach(otherFaceIdx => {
				if (otherFaceIdx === faceIdx) return;
				potentialNeighbors[otherFaceIdx] = (potentialNeighbors[otherFaceIdx] || 0) + 1;
			});
		});

		neighborMap[faceIdx] = Object.keys(potentialNeighbors)
			.filter(otherIdx => potentialNeighbors[otherIdx] >= 2)
			.map(Number);
	});

	return neighborMap;
}
function getFaceNeighborsAll(faceList) {
  const vertexToFaces = {};
  const neighborMap = {};

  // 1. Create an inverted index: which faces use which vertex?
  faceList.forEach((face, faceIdx) => {
    face.forEach(vertIdx => {
      if (!vertexToFaces[vertIdx]) {
        vertexToFaces[vertIdx] = [];
      }
      vertexToFaces[vertIdx].push(faceIdx);
    });
  });

  // 2. For each face, collect all unique neighbors from its vertices
  faceList.forEach((face, faceIdx) => {
    const neighbors = new Set();

    face.forEach(vertIdx => {
      const sharingFaces = vertexToFaces[vertIdx];
      
      sharingFaces.forEach(otherFaceIdx => {
        // Don't add the face itself as its own neighbor
        if (otherFaceIdx !== faceIdx) {
          neighbors.add(otherFaceIdx);
        }
      });
    });

    // Convert the Set back to an Array for the final map
    neighborMap[faceIdx] = Array.from(neighbors);
  });

  return neighborMap;
}
function logVerts(arr) {
	arr.forEach((element, index) => {
		// Stringifying helps ensure the console doesn't collapse the array
		console.log(`${index}: ${JSON.stringify(element)}`);
	});
}

// Usage: logVerts(mesh.vert_list);
function drawTessagonVertices(tessagon, width, height, radius = 15) {
	const { mesh, svg } = tessagon.create_mesh(); //console.log('mesh', mesh);
	const vertList = mesh.vert_list || [];

	if (vertList.length === 0) {
		return `<svg width="${width}" height="${height}"><text y="20">No vertices found.</text></svg>`;
	}

	const margin = radius * 2 + 20;
	const innerWidth = width - (margin * 2);
	const innerHeight = height - (margin * 2);

	let svgParts = [];
	svgParts.push(`<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">`);
	svgParts.push('<rect width="100%" height="100%" fill="#1a1a1a" />');

	// Background Grid
	svgParts.push('<g stroke="#333" stroke-width="1" fill="none">');
	for (let i = 0; i <= tessagon.u_num; i++) {
		const x = margin + (i / tessagon.u_num) * innerWidth;
		svgParts.push(`<line x1="${x}" y1="${margin}" x2="${x}" y2="${height - margin}" />`);
	}
	for (let j = 0; j <= tessagon.v_num; j++) {
		const y = margin + (j / tessagon.v_num) * innerHeight;
		svgParts.push(`<line x1="${margin}" y1="${y}" x2="${width - margin}" y2="${y}" />`);
	}
	svgParts.push('</g>');

	// Vertices with Centered Text
	vertList.forEach((coords, index) => {
		const cx = margin + (coords[0] * innerWidth);
		const cy = margin + (coords[1] * innerHeight);
...
function exportTessagonToSVGPlusTiles(tessagon, width, height) {
	// 1. SAFETY CHECKS
	if (!tessagon.tile_generator || !tessagon.tile_generator.tiles) {
		throw new Error("Tessagon Error: No tiles found. Ensure you called .create_mesh() before exporting.");
	}

	const adaptor = tessagon.mesh_adaptor;
	if (!adaptor || !adaptor.vert_list || !adaptor.face_list) {
		throw new Error("Tessagon Error: The mesh adaptor is missing vertex or face data.");
	}
	...
	// Add a background
	svgParts.push(`<rect width="100%" height="100%" fill="#f8f8f8" />`);

	// 2. DRAW THE TILES (The organizational grid)
	// These are the logical boundaries the library uses to partition space.
	svgParts.push('<g id="tile-boundaries" fill="none" stroke="#ff4444" stroke-width="1" stroke-dasharray="4" opacity="0.6">');

	const tiles = tessagon.tile_generator.tiles;
	for (let u = 0; u < tiles.length; u++) {
		if (!tiles[u]) continue;
		for (let v = 0; v < tiles[u].length; v++) {
			const tile = tiles[u][v];
			if (!tile || !tile.corners) continue;

			// corners in tess.js are [BottomLeft, BottomRight, TopLeft, TopRight]
			// For a polygon path, we need to visit them in order: BL -> BR -> TR -> TL
			const c = tile.corners;
			const points = [
				`${c[0][0] * width},${c[0][1] * height}`, // BL
				`${c[1][0] * width},${c[1][1] * height}`, // BR
				`${c[3][0] * width},${c[3][1] * height}`, // TR
				`${c[2][0] * width},${c[2][1] * height}`  // TL
			].join(' ');

			svgParts.push(`<polygon points="${points}" />`);

			// Optional: Label the tile index (U, V)
			const labelX = c[0][0] * width + 4;
			const labelY = c[0][1] * height + 12;
			svgParts.push(`<text x="${labelX}" y="${labelY}" fill="#cc0000" font-family="sans-serif" font-size="8">${u},${v}</text>`);
		}
	}
	svgParts.push('</g>');

	// 3. DRAW THE FACES (The actual geometric tessellation)
	// These often span across multiple tiles.
	svgParts.push('<g id="tessellation-faces" fill="rgba(64, 128, 255, 0.2)" stroke="#222" stroke-width="1.5" stroke-linejoin="round">');

	adaptor.face_list.forEach(faceIndices => {
		// faceIndices is an array of pointers into adaptor.vert_list
		const points = faceIndices.map(index => {
			const coords = adaptor.vert_list[index];
...
function exportTessagonToSVG(tessagon, width, height, palette = null) {
	// 1. DEFAULT PALETTE
	// Maps index 0, 1, 2... to specific colors.
	// Adjust these hex codes to match your design.
	const colors = palette || {
		0: '#e0e0e0', // Light Gray (Default/Background)
		1: '#4080ff', // Blue
		2: '#ff4080', // Pink/Red
		3: '#40ff80'  // Green
	};
	...
	// 3. DRAW THE TILES (Optional Debug Layer)
	svgParts.push('<g id="tile-boundaries" fill="none" stroke="#ccc" stroke-width="0.5" stroke-dasharray="2" opacity="0.3">');
	const tiles = tessagon.tile_generator.tiles;
	for (let u = 0; u < tiles.length; u++) {
		for (let v = 0; v < tiles[u].length; v++) {
			const tile = tiles[u][v];
	...
	// 4. DRAW THE FACES (With Color Patterns)
	svgParts.push('<g id="tessellation-faces" stroke="#333" stroke-width="1" stroke-linejoin="round">');

	adaptor.face_list.forEach((faceIndices, i) => {
		// Retrieve the color index for this specific face
		// This was generated by your color_pattern1() or color_pattern2()
		const colorIndex = (adaptor.color_list && adaptor.color_list[i] !== undefined)
			? adaptor.color_list[i]
			: 0;
		...
		// Apply the color pattern to the fill attribute
		//svgParts.push(`<polygon points="${points}" fill="${fillColor}" />`);
		// Inside the face_list loop of exportTessagonToSVG:
		svgParts.push(`<polygon points="${points}" fill="${fillColor}" data-index="${i}" class="tess-face" />`);
	});
...
function labelVertices(mesh, svg) {
	svg = toElem(svg);
	const svgNS = "http://www.w3.org/2000/svg";

	mesh.vert_list.forEach((point, id) => {
		const [x, y] = point;

		const text = document.createElementNS(svgNS, "text");
		text.setAttribute("x", x);
		text.setAttribute("fill", "white");
		// Offset y slightly so the text doesn't sit exactly on the circle
		text.setAttribute("y", y); // - 0.02);
		text.setAttribute("font-size", "0.015");
		text.setAttribute("text-anchor", "middle");
		text.setAttribute("dominant-baseline", "central");
		text.textContent = id; // The index in vert_list

		svg.appendChild(text);
	});
}
function drawEdges(mesh, svgId) {
	...
	mesh.face_list.forEach(faceIndices => {
		// A face is an array of vertex indices in winding order
		for (let i = 0; i < faceIndices.length; i++) {
			const v1Idx = faceIndices[i];
			const v2Idx = faceIndices[(i + 1) % faceIndices.length]; // Wrap to start

			// Create a unique key (e.g., "5_12") so we don't draw shared edges twice
			const edgeKey = v1Idx < v2Idx ? `${v1Idx}_${v2Idx}` : `${v2Idx}_${v1Idx}`;

			if (!edgeSet.has(edgeKey)) {
				edgeSet.add(edgeKey);

				const p1 = mesh.vert_list[v1Idx];
				const p2 = mesh.vert_list[v2Idx];
				...
				// Styling for 0 to 1 scale
				line.setAttribute("stroke", "red"); // Dark grey
				line.setAttribute("stroke-width", "0.02"); // Very thin for 0-1 scale
...
function drawEdges(mesh, svgId) {
	...
	const svgNS = "http://www.w3.org/2000/svg";
	const edgeCounts = new Map(); // Stores "edgeKey" => count
	const edgeData = new Map();   // Stores "edgeKey" => [v1Idx, v2Idx]

	// 1. First pass: Count how many faces use each edge
	mesh.face_list.forEach(faceIndices => {
		for (let i = 0; i < faceIndices.length; i++) {
			const v1Idx = faceIndices[i];
			const v2Idx = faceIndices[(i + 1) % faceIndices.length];
			const edgeKey = v1Idx < v2Idx ? `${v1Idx}_${v2Idx}` : `${v2Idx}_${v1Idx}`;

			edgeCounts.set(edgeKey, (edgeCounts.get(edgeKey) || 0) + 1);
			if (!edgeData.has(edgeKey)) {
				edgeData.set(edgeKey, [v1Idx, v2Idx]);
			}
		}
	});

	// 2. Second pass: Draw the lines
	edgeData.forEach((indices, key) => {
		const count = edgeCounts.get(key);
		const [v1Idx, v2Idx] = indices;
		const p1 = mesh.vert_list[v1Idx];
		const p2 = mesh.vert_list[v2Idx];
		...
		// Boundary Logic:
		// count === 1 means it's an edge on the outer border of the board.
		// count === 2 (or more) means it's an internal divider.
		if (count === 1) {
			line.setAttribute("stroke", "black");      // Border color
			line.setAttribute("stroke-width", "0.01"); // Slightly thicker
		} else {
			line.setAttribute("stroke", "#999");        // Internal color (grey)
...
function addCircle(svgId, x, y, radius = 5, color = "red", id) {
	const svgNS = "http://www.w3.org/2000/svg";
	const svgElement = toElem(svgId); //document.getElementById(svgId);

	// 1. Create the circle element with the SVG namespace
	...
	circle.setAttribute("data-id", id); // This is the vertex ID
	// 3. Append to the SVG
	svgElement.appendChild(circle);
}
function buildTopology(mesh) {
	const topology = {
		// For a circle (vertex): adjacent edges and adjacent faces
		circle_edges: {},  // id -> [edgeId1, edgeId2, ...]
		circle_faces: {},  // id -> [faceId1, faceId2, ...]

		// For a face: adjacent circles and adjacent edges
		face_corners: {},  // id -> [circleId1, circleId2, ...]
		face_edges: {},    // id -> [edgeId1, edgeId2, ...]

		// For an edge: the two circles it connects and the faces it separates
		edge_data: {}      // edgeId -> { circles: [c1, c2], faces: [f1, f2] }
	};

	const edgeMap = new Map(); // internal helper: "v1_v2" -> edgeId
	let edgeCounter = 0;

	// Process Faces
	mesh.face_list.forEach((faceVertices, faceId) => {
		topology.face_corners[faceId] = faceVertices;
		topology.face_edges[faceId] = [];

		// Track faces connected to each vertex
		faceVertices.forEach(vId => {
			if (!topology.circle_faces[vId]) topology.circle_faces[vId] = [];
			topology.circle_faces[vId].push(faceId);
		});

		// Process Edges within this face
		for (let i = 0; i < faceVertices.length; i++) {
			...
			if (!edgeMap.has(edgeKey)) {
				...
				// Link circle to edge
				[v1, v2].forEach(vId => {
					if (!topology.circle_edges[vId]) topology.circle_edges[vId] = [];
					topology.circle_edges[vId].push(edgeId);
				});
			} else {
				edgeId = edgeMap.get(edgeKey);
				// This edge is shared; add the second face
				topology.edge_data[edgeId].faces.push(faceId);
			}

			// Link face to edge
			topology.face_edges[faceId].push(edgeId);
		}
	});

	return topology;
}
function labelEdges(mesh, topology, svgId) {
	const svg = document.getElementById(svgId);
	Object.entries(topology.edge_data).forEach(([edgeId, data]) => {
		const p1 = mesh.vert_list[data.circles[0]];
		const p2 = mesh.vert_list[data.circles[1]];
		...
		text.setAttribute("font-size", "0.006");
		text.textContent = edgeId;
		svg.appendChild(text);
	});
}
function getNeibors(faceId, tessagon) {
	console.log(faceId, tessagon)
	const face = tessagon.mesh.face_instances[faceId];
	if (!face) return [];
	let results = [];
	// Walk through all edges of the current face
	face.edge_instances.forEach(edge => results.push(edge));
	return results;

}
function getFacesByType(faceId, targetType, tessagon) {
	const face = tessagon.mesh.face_instances[faceId];
	if (!face) return [];

	let results = [];
	// Walk through all edges of the current face
	face.edge_instances.forEach(edge => {
		// Check both faces connected to this edge
		edge.face_instances.forEach(neighbor => {
			// 1. Don't include the face we started with
			// 2. Check if the metadata 'face_type' matches
			const type = neighbor.opts.face_type;
			if (neighbor !== face && type === targetType) {
				if (!results.find(f => f.id === neighbor.id)) {
...
// 1. Get the 'Streets' (Squares) surrounding a Hex or connected to a City
function getNeighboringRoads(faceId, tessagon) {
	return getFacesByType(faceId, 'road', tessagon);
}

// 2. Get the 'Settlements/Cities' (Triangles) at the ends of a Road 
// or corners of a Hex
function getNeighboringCities(faceId, tessagon) {
	return getFacesByType(faceId, 'node', tessagon);
}

// 3. Get the 'Resource Tiles' (Hexagons) adjacent to a Road or City
function getNeighboringTiles(faceId, tessagon) {
	return getFacesByType(faceId, 'hex', tessagon);
}
...
function onclickSvg(e, tessagon) {
	const faceEl = e.target.closest('.tess-face');
	let info = {
		id: faceEl.dataset.id,
		colorIndex: parseInt(faceEl.dataset.colorIndex),
		cx: parseFloat(faceEl.dataset.cx),
		cy: parseFloat(faceEl.dataset.cy),
		element: faceEl,
		event: e,
		dataset: faceEl.dataset,
		neibors: getNeibors(faceEl.dataset.id, tessagon),
	};
	console.log('faceEl', info); return;
	if (!faceEl) return;

	// Toggle a selection class (defined in your CSS/Style block)
	faceEl.classList.toggle('selected');
...
function generateSvgTessellation(cols = 10, rows = 10, name = 'HexTessagon', opts = {}) {

	const options = {
		function: planeFunction,
		u_range: [0.0, 1.0], //xmin,xmax
		v_range: [0.0, 1.0], //ymin,ymax
		u_num: cols, //#tiles
		v_num: rows,
		u_cyclic: false,
		v_cyclic: false,
	...
	let shapeFunc = getTessagonDict()[name];
	const tessagon = new shapeFunc(options);
	const mesh = tessagon.create_mesh();
	const svg = mesh;
	console.log("internal mesh graph", tessagon.mesh);
	//const svg = tessagon.create_mesh();
	return { tessagon, svg };
}

function wrapGElements(gElements, id, stroke = 'white') {
	// We use viewBox="0 0 1 1" because Tessagon uses normalized 0.0 to 1.0 coordinates
    ...
    <svg
     id="${id}"  
      ...
      <style>
        /* Optional: ensures lines are thin regardless of zoom */
        polygon { 
          stroke: ${stroke}; 
          stroke-width: 1px; 
...
function genTess(dParent, rows, cols, name, opts = {}, func, id = null) {
	// 1. Generate a unique ID if one isn't provided
	//const svg_id = opts.svg_id || `tess_${Math.floor(Math.random() * 10000)}`;

	// 2. Auto-detect the color method based on provided options
	if (!opts.color_method) {
		if (opts.svg_fill_colors) {
			opts.color_method = 'palette';
			if (opts.color_pattern === undefined) opts.color_pattern = 1;
		} else if (opts.svg_fill_color) {
			opts.color_method = 'solid';
		} else {
			opts.color_method = 'hsl';
		}
	}

	// 3. Generate the geometry and groups
	let { tessagon, svg } = generateSvgTessellation(rows, cols, name, opts);

	// 4. Inject into DOM using your updated wrapper function
	dParent.innerHTML = ''; // Clear previous attempts
	dParent.innerHTML = wrapGElements(svg, id ?? `tess_${Math.floor(Math.random() * 10000)}`);
	//dParent.innerHTML = wrapGElements(svg, svg_id);

	// 5. Select the container and attach interaction logic
	const svgContainer = dParent.querySelector(`#${id}`);
	if (!svgContainer) return { tessagon, svg };
	...
	// 	const faceEl = e.target.closest('.tess-face');
	// 	if (faceEl && opts.onhover) {
	// 		opts.onhover({ id: faceEl.dataset.id, element: faceEl });
	// 	}
	// });
	svgContainer.addEventListener('mouseover', (e) => {
		const faceEl = e.target.closest('.tess-face');
		if (faceEl) {
			// TEMPORARY: Highlight the actual element the mouse is hitting
			faceEl.style.outline = "2px solid red";
			//setTimeout(() => faceEl.style.outline = "none", 500);

			//if (opts.onhover) opts.onhover({ id: faceEl.dataset.id, element: faceEl });
		}
	});
	return { tessagon, svgContainer };
}

function updateTessColors(svgElement, newPalette) {
	const faces = svgElement.querySelectorAll('.tess-face');
	faces.forEach(face => {
		const idx = parseInt(face.dataset.colorIndex);
		if (!isNaN(idx)) {
			face.setAttribute('fill', newPalette[idx % newPalette.length]);
		}
	});
}
...
function generateAllTessellationVariants(cols = 10, rows = 10) {
	const tessagonDict = getTessagonDict();
	const results = [];

	// A robust palette to ensure we cover tessellations 
	// that use indices 0, 1, 2, and 3
	const defaultPalette = ['#264653', '#2a9d8f', '#e9c46a', '#f4a261', '#e76f51'];

	for (const name in tessagonDict) {
		const Klass = tessagonDict[name];

		// Get the number of patterns from the static metadata method we fixed
		// If num_color_patterns is 1, we usually want to test pattern 1.
		// If it's 0, we just run the default.
		const numPatterns = Klass.num_color_patterns();

		// Always run at least once (pattern 0 or default)
		const patternsToRun = numPatterns === 0 ? [0] : Array.from({ length: numPatterns }, (_, i) => i + 1);

		patternsToRun.forEach(patternIndex => {
			console.log(`Generating ${name} with color_pattern: ${patternIndex}`);

			const options = {
				function: planeFunction, // Assuming planeFunction is in your scope
				u_range: [0.0, 1.0],
				v_range: [0.0, 1.0],
				u_num: cols,
				v_num: rows,
				...
				v_cyclic: false,
				adaptor_class: SvgAdaptor,
				color_pattern: patternIndex,
				svg_fill_colors: defaultPalette,
				svg_stroke_color: '#333',
				svg_stroke_width: 0.002,
				svg_root_tag: true
			};
			if (name == 'WeaveTessagon') {
				//console.log('HAAAAAAAAAAAAAAAAAAAAAAAAAA')
				options.u_cyclic = true;
				options.v_cyclic = true;
			}

			try {
				const tessagon = new Klass(options);
				const svg = tessagon.create_mesh();
				...
				console.error(`Failed to generate ${name} pattern ${patternIndex}:`, e);
			}
		});
	}
